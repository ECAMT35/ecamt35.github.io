...
当 `LOC_ID` 列上存在非聚簇索引时，数据库优化器在处理这种由多个 `OR` 连接的等值条件时，**生成的执行计划可能不够高效**。一种可能的情况是，优化器选择为每个 `OR` 条件**分别执行一次索引查找**（Index Seek）。也就是说，执行引擎会依次以 `10`、`20`、`30` 作为键值，在 `LOC_ID` 的索引结构上进行三次独立的查找操作，每次获取到符合单个条件的主键集合，最后再将这三个中间结果集合并，并回表获取完整数据行。

**这种执行方式本质上是将一次数据请求分解成了多次独立的索引访问和结果合并，增加了额外的开销。**

>注意 SQL 查询和索引查询的区别
>SQL 查询是整个 SQL 语句的流程查询，索引查询是拿着这个条件去直接问某个字段的索引

**高效写法建议**

使用 `IN` 列表通常能获得更优的执行计划：

```sql
select * from t where LOC_ID in (10, 20, 30);
```

现代数据库（如 MySQL）的优化器会对 `IN` 子句进行专门优化。它会将 `IN` 列表中的常量值存储在一个**有序**数组中，然后**通过一次有序的索引范围扫描或多次合并的查找，在一次操作中完成所有值的匹配**。这避免了为每个值发起独立查找操作的开销，大大提升了效率。

#### 执行计划对比

可能会问，or 分多次去问索引，那 in 列表的值也不止一个啊，不用去一个个对比吗？

首要要记得，这个数据结构是 b+ 树，可以从执行计划的角度去看：
`or` 的大概执行计划：

>动作 1: 处理 LOC_ID 列的多个等值 OR 条件。动作 2: 为 LOC_ID=10 执行索引查找：
>- 从根节点定位到叶子节点中 10 的起始位置；
>- 扫描所有 10 的叶子节点条目，收集对应的主键；
>- 结束本次索引查找，不立即回表。
>动作 3: 为 LOC_ID=20 重复动作 2，重新从根节点定位，收集主键。
>动作 4: 为 LOC_ID=30 重复动作 2，收集主键。
>动作 5: 将三次收集的主键合并、去重，批量回表获取完整行数据。

每次查找都是独立的、有“起止”的过程，产生了重复的**定位成本**（从根节点向下搜索）。

`IN` 的大概执行计划：

>动作 1: 将 IN (10,20,30) 中的值排序为有序数组 [10,20,30]，记录最小值 min=10、最大值 max=30。
>动作 2: 执行一次索引范围扫描：
>- 从根节点定位到叶子节点中 min=10 的起始位置；
>- 从 10 开始，向右顺序扫描叶子节点；
  动作 3: 扫描过程中，用有序数组与叶子节点的索引键做匹配：
>- 若叶子节点的键在数组中，收集对应的主键；
>- 若叶子节点的键不在数组中，直接跳过；
>- 当扫描到的键超过 max=30，立即停止扫描。    
>动作 4: 将收集到的主键批量回表，获取完整行数据。

整个过程是**一次连续的、单向的索引叶子节点遍历**。优化器拿着排序好的数组，像合并两个有序链表一样，在索引顺序扫描的过程中一次性完成所有值的匹配。这**避免了为每个值重新从根节点开始定位的开销**。
