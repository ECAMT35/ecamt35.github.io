...
在业务线程中，已经脱离了 Netty 的 IO 线程。

### 切回 NettyIO 线程操作 channel

如果在业务线程池里需要对 cahnnel 的 attrs 操作，或者要使用 channel 发送一些消息给客户端之类的，必须要切回这个 channel 所在 Netty 的 IO 线程来完成串行操作，避免并发下执行错误顺序的带来不好的副作用。
...
### 不要以 `ChannelHandlerContext` 作为传递参数

原因：
- `ChannelHandlerContext` 与 `ChannelHandler` 的生命周期强绑定。如果 Handler 被移除或 Channel 关闭，Context 可能失效，继续使用会导致资源泄漏或异常，不适合跨线程传递。
- 将 Context 传递到业务层会让网络层逻辑泄漏到业务层，违反单一职责原则。
- `Channel` 是长生命周期的抽象，提供线程安全的操作，是跨线程传递的正确选择。
